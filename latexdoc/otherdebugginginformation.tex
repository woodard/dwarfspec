...
\bb
\item An optional local string pool.
\eb
...
\begin{tikzpicture}[
  every node/.style={node font=\small, anchor=north west, text height=.8em,
                     text depth=.2em, inner sep=4pt, outer ysep=0},
  caption/.style={node font=\small \bfseries, text width=90pt},
  overview/.style={draw, node font=\small, minimum height=28pt, text width=90pt},
  detail1/.style={draw, minimum height=14pt, text width=116pt},
  detail2/.style={draw, minimum height=28pt, text width=116pt},
  detail3/.style={draw, minimum height=30pt, text width=116pt},
  detail4/.style={draw, minimum height=40pt, text width=116pt, align=left},
...
\begin{scope}[start chain=going below, node distance=0, yshift=-150pt]
  ...
  \node (localstrs)[on chain,overview,header] {Local String Pool};
  \node (abbrev)   [on chain,overview,header] {Abbrev Table};
  \node (pool)     [on chain,overview,header] {Index Pool};
\end{scope}
...
\begin{scope}[on background layer]
  \filldraw [explode] (culist.north east) -- (cu0.north west) --
                       (cu3.south west) -- (culist.south east) -- cycle;
\end{scope}
...
\begin{scope}[on background layer]
  \filldraw [explode] (ltulist.north east) -- (ltu0.north west) --
                      (ltu3.south west) -- (ltulist.south east) -- cycle;
\end{scope}
...
\begin{scope}[on background layer]
  \filldraw [explode] (ftulist.north east) -- (ftu0.north west) --
                      (ftu3.south west) -- (ftulist.south east) -- cycle;
\end{scope}
...
\begin{scope}[on background layer]
  \filldraw [explode] (hash.north east) -- (hash0.north west) --
                      (hash1.south west) -- (hash.south east) -- cycle;
\end{scope}

% Exploded View of Name Table

\begin{scope}[start chain=going below, node distance=0, shift={($(hash1.south west) + (0,-9pt)$)}]
  \node (name0) [on chain,detail4,stroffsets]   {\begin{tabular}{l}
                                                   String Pointers\\
												   or Indexes
												 \end{tabular}};
...
\begin{scope}[on background layer]
  \filldraw [explode] (names.north east) -- (name0.north west) --
                      (name1.south west) -- (names.south east) -- cycle;
\end{scope}

% Exploded View of Local Strings Pool

\begin{scope}[shift={($(name1.south west) + (0,-9pt)$)}]
  \node (localstrs0) [detail3,localstrs] {Local Strings};
\end{scope}

begin{scope}
\filldraw [explode] (localstrs.north east) -- (localstrs0.north west) --
                    (localstrs0.south west) -- (localstrs.south east) -- cycle;

% Exploded View of Abbreviations

\begin{scope}[shift={($(localstrs0.south west) + (0,-9pt)$)}]
  \node (abbrev0) [detail3,abbrevs] {Abbreviations};
\end{scope}

\begin{scope}[on background layer]
  \filldraw [explode] (abbrev.north east) -- (abbrev0.north west) --
                      (abbrev0.south west) -- (abbrev.south east) -- cycle;
\end{scope}
...
\bb
\item \HFNstrformat{} (\HFTubyte) \\
An enumerated constant that specifies the representation of
string references in the name index. The possible values are:
\DWFORMstrp, \DWFORMstrpeight, and \DWFORMstrxfour{} (see
Section \refersec{datarep:classesandforms}).

\item \HFNpadding{} (\HFTubyte) \\
\eb
...
\bb
\item \HFNlocalstrpoolsize{} (\HFTsectionlength) \\
Size of the local string pool. If this value is non-zero,
string offsets (when \HFNstrformat{} is \DWFORMstrp{} or \DWFORMstrpeight)
reference the local string pool. If this value is 0,
string offsets reference the \dotdebugstr{} section.
If \HFNstrformat{} is \DWFORMstrxfour, this field should be 0.

\item \HFNstroffsets{} (\HFNsectionoffset) \\
A 4-byte or 8-byte unsigned offset that points to the header
of the compilation unit’s contribution to the \dotdebugstroffsets{}
section. Indirect string references (when \HFNstrformat{} is
\DWFORMstrxfour) are interpreted as zero-based indexes into the
array of offsets following the header. If \HFNstrformat{} is
\DWFORMstrp{} or \DWFORMstrpeight, this field should be 0.
\eb
...
\bb
The name table immediately follows the hash lookup table.
It consists of two arrays: an array of string pointers or indexes,
followed immediately by an array of entry offsets. The items in
the first array are determined by the \HFNstrformat{} field in the
section header, and may be 4-byte or 8-byte offsets into either the
\dotdebugstr{} section or the local string pool, or 4-byte indexes
into the array of offsets in the \dotdebugstroffsets{} section.
The items in the second array are section offsets: 4-byte unsigned
integers for the DWARF-32 format or 8-byte unsigned integers for the
DWARF-64 format. The entry offsets in the second array refer to index
entries, and are relative to the start of the entry pool area.
\eb
...
\bb
\subsubsubsection{Local String Pool}
\label{chap:localstringpool}
The local string pool, if present, immediately follows the name table.
It consists of a series of null-terminated strings. Its size is given by
\HFNlocalstrpoolsize.

\textit{For non-split DWARF compilation units, strings used by the
name table will have significant overlap with strings used by the
\dotdebuginfo{} section, and a local string pool is not advisable.
Relocations for the string references may be minimized by using the
indirect string forms in both \dotdebuginfo{} and \dotdebugnames.
For split DWARF compilation units with a linker that is aware of
and can combine \dotdebugnames{} sections into a single per-module
index, there is likely little overlap, and relocations for string
references in the name table can be minimized by using the local
string pool. If the linker simply concatenates the per-CU indexes,
however, it remains beneficial to use indirect string forms and a
separate string table.}
\eb

\needlines{6}
\subsubsubsection{Abbreviations Table}
The abbreviations table immediately follows
\bb
the local string pool or, if the local string pool is absent,
\eb
the name table. This table
...
\db	% Delete Section 6.1.2 Lookup By Address
...
\LNSREGaddress
\addttindexx{address register!in line number machine}&
...
0, and the \addtoindex{operation pointer} is simply the \LNSREGaddress{} register.
...
fnad{} and \dotdebuglinestr) from an executable.}
...
because no
\bb
\doublequote{.debug\_line\_str\_offsets.dwo} or
\doublequote{.debug\_line\_str.dwo}
\eb
...
expressions (see Section \refersec{chap:generaloperations}). The DWARF
expressions for call frame information are restricted to +> those
operations that do not require a current compilation unit (see Section
\refersec{chap:expressionevaluationcontext)}.  The following DWARF
