...
%
\newcommand{\docdate}{October 12, 2024}
\newcommand{\thisversion}{6}
%
\usepackage{ifthen}
    \newcommand{\ifthen}[2]{\ifthenelse{#1}{#2}{}}
    \newboolean{isdraft}\setboolean{isdraft}{true}
    \newboolean{uselinenumbers}\setboolean{uselinenumbers}{false}
    \newboolean{usechangesummary}\setboolean{usechangesummary}{true}
\newcommand{\draftmark}{\ifthenelse{\boolean{isdraft}}{***WORKING DRAFT***}{}}
%
...
\input{dwarfchangebarpatch}% Patch needed for package changebar
...
\usepackage[color]{changebar}     % For change bars in margin
    \cbcolor{red}
...
% Standard formats for "New in DWARF Version x" annotations
\newcommand{\newinversionx} 	{new in DWARF Version \thisversion}
\newcommand{\Newinversionx} 	{New in DWARF Version \thisversion}
\newcommand{\ddagnewinversionx} {\ddag~New in DWARF Version \thisversion}
...
% allow a word to appear in tt font in the main text and the index
...
\newcommand{\bbeb}{\bb\eb}		% Single line bar (without spanning text); useful in tables
\newcommand{\db}{\cbdelete}     % Mark deleted word or phrase
% Mark deleted paragraph
\newcommand{\bbpareb}{\vspace{-0.5\baselineskip}\db\vspace{-0.5\baselineskip}} 
...
\\
\bb
\textbf{Version \thisversion}
\eb
\\
\vspace{1cm}
\includegraphics[keepaspectratio=true,scale=0.8]{dwarf6-new-logo}
\\
\vspace{1cm}
...
% Enable or comment out the empty page after each chapter so the
% the following chapter starts on an odd numbered page
%
