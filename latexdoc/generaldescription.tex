...
\textit{Note that this compilation unit might not be the same as the
  ...
  and not the one that contains the \DWOPcallX{} operation
  expression.}
...
LEB128\addtoindexx{LEB128!signed} offset from the address specified by
the location description in the \DWATframebase{} attribute of the
current function. (see Section \refersec{chap:expressionevaluationcontext})
...
\refersec{chap:expressionevaluationcontext}). This object may
...
into an address in the t\addtoindex{hread-local storage} for the
current thread (see Section \refersec{chap:expressionevaluationcontext})
...
(see Section \refersec{chap:expressionevaluationcontext} and Section
\refersec{chap:callframeinformation}).
...
The \DWOPpushlaneNAME{} operation pushes a lane index value of the
generic type, which provides the context of the lane in which the
expression is being evaluated.  See The operation pushes the value of
the CFA, obtained from the Call Frame Information (see Section
\refersec{chap:expressionevaluationcontext} and Section
\refersec{chap:lowlevelinformation}.
