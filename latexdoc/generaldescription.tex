...
\DWATbiasTARG
\bb
&\livelinki{chap:DWATbiassimpleencodingbias}
...
&\livelinki{chap:DWATlanguageprogramminglanguagename}
        {Programming language name}
        {programming language name} \\*
\DWATlanguageversionTARG
&\livelinki{chap:DWATlanguageprogramminglanguageversion}
...
\bbeb
\DWATnumlanesTARG
&\livelinki{chap:lanesinsimdvectorization}
...
&\livelinki{chap:DWATscaledivisorofscalefactor}
		{Denominator of rational scale factor}
		{denominator of rational scale factor} \\
\bbeb
\DWATscalemultiplierTARG
&\livelinki{chap:DWATscalemultiplierofscalefactor}
...
\DWATstroffsetsTARG\footnote{
	...
	DW\_AT\_str\_offsets\_base\addtoindexx{DW\_AT\_str\_offsets\_base (deprecated)}
	which is deprecated.}
&\livelinki{chap:DWATstroffsetforindirectstringsection}
        {String offsets information for unit}
        {string offsets information for unit}
\eb		\\
\bb
\DWATtensorTARG
&\livelinki{chap:DWATtensorarraytype}
		{Tensor (array) type}
		{Tensor (array) type}
\eb		\\
...
\livelinki{datarep:classexprval}{exprval}{exprval class}
&A DWARF expression yielding a value (see Section
\refersec{chap:dwarfexpressions}). A leading unsigned
ULEB128 value (see Section \refersec{datarep:variablelengthdata})
...
\livelinki{datarep:classlocdesc}{locdesc}{locdesc class}
...
\livelinki{datarep:classvallist}{vallist}{vallist class},
\eb
...
\addtoindexx{reserved target address}
\addtoindexx{target address!reserved}
...
\textit{The generic type is the same as the unspecified type used for
...
\item Single location descriptions, which are a language-independent
  ...
  location of any object as long as its lifetime is either static or
  the same as the lexical block that owns it, excluding any prologue
  or epilogue ranges, and it does not move during its lifetime.

\item Location lists, which are used to describe objects that have a
  limited lifetime or change their location during their lifetime.
  Location lists are described in Section /referspec{chap:locationlists}.
...
using \hypertarget{chap:classlocdesc}{}
\livelinki{datarep:classlocdesc}{locdesc}{class locdesc}, and a
location list is encoded using \hypertarget{chap:classloclist}{}
\livelinki{datarep:classloclist}{loclist}{class loclist} (which serves
as an index into a separate section containing location lists).
...
\subsection{DWARF Expression Evaluation Context}
\label{chap:expressionevaluationcontext}
DWARF expressions and location descriptions (see
\refersec{chap:locationdescriptions}) are evaluated within a context
provided by the debugger or other DWARF consumer. The context includes
the following elements:

\begin{enumerate}[1. ]

\itembfnl{Required result kind} The kind of result required
\textemdash either a location or a value \textemdash is determined by
the DWARF construct where the expression is found.

\textit{For example, DWARF attributes with exprval class require a
  value, and attributes with locdesc class require location
  description (see Section \refersec{datarep:classesandforms}).}

\itembfnl{Initial stack}
...
expression starts (e.g. \DWATdatamemberlocation).

\itembfnl{Current compilation unit}
...
\textit{For example, the \DWOPconstx{} and \DWOPaddrx{} operations
  require the address size, which is a property of the compilation
  unit.}
...
\textit{Note that this compilation unit might not be the same as the
  ...
  attribute of the debug information entry operand of the \DWOPcallstar{}
  operations is evaluated with the compilation unit that contains E
  and not the one that contains the \DWOPcallstar{} operation
  expression.}

\textit{The DWARF expressions for call frame information (see Section
  \refersec{chap:callframeinformation}) operations are restricted
  to those that do not require a current compilation unit.}

\itembfnl{Target architecture}
...
\itembfnl{Current thread}

\textit{Many programming environments support the concept of
  independent threads of execution, where the process and its address
  space are shared among the threads, but each thread has its own
  stack, program counter, and possibly its own block of memory for
  thread-local storage (TLS). These threads may be implemented in
  user-space or with kernel threads, or by a combination of the two.}
...
core file), there is no current thread. A single-threaded program (as
distinguished from a multi-threaded program with only a single thread)
might not have a current thread.
...
stack (see Section \refersec{chap:framebase}). The address is referred
...
\refersec{chap:callframeinformation}).
...
\itembfnl{Current lane}

\textit{On SIMD (Single-Instruction Multiple-Data Stream) and SIMT
  (Single-Instruction Multiple-Thread) architectures, fine-grained
  parallel execution can be achieved by dispatching a single
  instruction across multiple data streams (e.g., a vector or
  array). Some parallel programming models allow for the vectorization
  of loops using SIMD instructions. These parallel streams can be
  considered fine-grain threads of execution, or lanes, where each
  lane typically shares a common stack, program counter, and register
  file.}

\textit{In SIMT architectures, control flow may diverge through the
  use of predication, where each instruction executes only in certain
  lanes. Some SIMT architectures, however, provide separate stacks and
  register files for each lane, and the parallel streams of execution
  would be better represented as threads (above).}
...
\refersec{chap:lanesinsimdvectorization}). When debugging
...
If there is no running process, there is no current lane. If the
current running program is not using a SIMD/SIMT execution model, the
current lane is always 0.

\itembfnl{Current program counter (PC)}
...
\textit{When evaluating value lists and location lists when no current
  pc is available, only default location descriptions are used.}

\itembfnl{Current object}
...
and by some attributes (e.g., \DWATdatamemberlocation{} and
\DWATuselocation) where the object's location is provided as part of
the initial stack.
...
The following \addtoindexx{DWARF expression!stack operations}
...
See section \refersec{chap:lowlevelinformation}.
...
signed LEB128\addtoindexx{LEB128!signed} offset from the address
...
A \addtoindexx{location description!memory} memory location
\addtoindexx{memory location} represents the location of a piece or
...
operations provides a signed LEB128\addtoindexx{LEB128!signed} byte
offset. The contents of the specified register (0–31) are treated as a
memory address in the default address space. The offset is added to
the address obtained from the register and the resulting memory
location is pushed onto the stack.

\itembfnl{\DWOPbregxTARG} The \DWOPbregxNAME{} operation provides a
memory location formed from its two operands. The first operand is a
register number which is specified by an unsigned
LEB128\addtoindexx{LEB128!unsigned} number. The contents of the
specified register are treated as a memory address in the default
address space. The second operand is a signed
LEB128\addtoindexx{LEB128!signed} byte offset. The offset is added to
the address obtained from the register and the resulting location is
...
The \DWOPderefNAME{} operation pops a location `L` from the top of the
stack. The first `S` bytes, where `S` is the size of an address on the
target machine, are retrieved from the location `L` and pushed onto
...
represented by thesecond operand. This operation pops a location `L`
from the stack. The first `S` bytes are retrieved from the location
`L` and pushed onto the stack as a value of type `T`.
...
An \addtoindex{implicit location} represents a piece or all
\addtoindexx{location!implicit} of an object which has no actual
...
A series of piece operations \DWOPpiece{} or \DWOPbitpiece{}
...
\textit{This operator is provided so that a new series of piece
  ...
  state of the stack is unknown (e.g., following a \DWOPcallstar{}
  operation).}

\itembfnl{\DWOPpieceTARG}
The \DWOPpiece{} operation takes a single operand, which is an
unsigned LEB128 \addtoindexx{LEB128!unsigned} number. The number
...
unsigned LEB128\addtoindexx{LEB128!unsigned} number that gives the
size S, in bits, of the piece. The second is an unsigned
LEB128\addtoindexx{LEB128!unsigned} number that gives the offset in
...
\refersec{chap:undefinedlocations}), the \DWOPbitpiece{} operation
...
address (see Section \refersec{chap:memorylocations}),
the \DWOPbitpiece{} operation describes a sequence of bits relative
...
Section \refersec{chap:registerlocations}) or an implicit value
location (see Section \refersec{chap:implicitlocations}); the offset
is from the least significant bit of the source value.

\DWOPbitpiece{} is used instead of \DWOPpiece{} when the piece
...
Whether or not a \DWOPpiece{} operation is equivalent to any
...
\addtoindexx{location attribute} the \DWATlocation{} attribute of the
...
\addtoindexx{location attribute} a \DWATlocation{} attribute may pop
...
(see Section \refersec{chap:registerlocations}).
...
along with location lists (see \refersec{chap:locationlists}).

A value list is indicated by an attribute whose value is of class
\label{chap:classvallist}\CLASSvallist{} (see Section \refersec{datarep:classesandforms}).

\needlines{6}
A value list consists of a series of value list entries.
The representation of a value list is the same as for a
location list (see \refersec{chap:locationlists}), except
...
\begin{itemize}
\item \definition{\livetargi{text:bndlocdesc}{Bounded location description}{bounded location description}}.
...
\refersec{chap:reservedtargetaddress}) indicates a non-existent range,
which is equivalent to omitting the description.
\eb

\item \definition{Default location description}.
\addtoindexx{default location description}
This kind of entry provides a
location description that specifies the location of
...
\needlines{6}
\item \definition{Base address}.
\addtoindexx{base address!of location list}
...
\item  \definition{End-of-list}.
\addtoindexx{end-of-list!of location list}
This kind of entry marks the end of the location list.

\end{itemize}

\textit{If there is no current PC (see Section
  \refersec{chap:expressionevaluationcontext}), only default location
...
This is a form of \livelink{text:bndlocdesc}{bounded location description} entry
\bb
...
A \addtoindexx{location description!simple}
...
The \addtoindex{address class} may affect the way a pointer
...
\refersec{chap:reservedtargetaddress}) for the non-existent entity.
\eb
...
\begin{itemize}
\item \definition{\livetargi{text:bndrng}{Bounded range}{bounded range}}.
\label{text:bndrng}
\bbeb
...
This is a form of \livelink{text:bndrng}{bounded range}
\bb
...
\item For a \livelink{chap:classreference}{reference}, the value
\bb
...
(see Section \refersec{chap:dataobjectentries}) or common block
(see Section \refersec{chap:commonblockentries}), the attribute
...
(see Section \refersec{chap:datalocationsanddwarfprocedures})
